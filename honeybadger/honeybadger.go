...
func (hb *HoneyBadger) HandleMessage(msg *pb.Message) error {
	...
	fmt.Println("test6")
	fmt.Printf("addr=%v\n",addr)
	fmt.Printf("hb.memberMap=%v\n",hb.memberMap)
	fmt.Printf("msg=%v\n",msg)
	//找出发送者里对应的member，应该是对应连接池中的节点
...
func (hb *HoneyBadger) handleBatchMessage(batchMessage cleisthenes.BatchMessage) error {
	...
	fmt.Println("test28")
	fmt.Printf("batchMessage.Epoch=%v",batchMessage.Epoch)
	fmt.Printf("decryptedBatch=%v",decryptedBatch)
